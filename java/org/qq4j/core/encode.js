...
function hash(uin, ptwebqq) {
    ...
    for (var i = 0; i < a.length; i++) {
        ...
        if (j + 4 > ptwebqq.length)
            for (var l = 4 + j - ptwebqq.length, x = 0; x < 4; x++)
                c |= x < l ? (ptwebqq.charCodeAt(j + x) & 255) << (3 - x) * 8 : (ptwebqq.charCodeAt(x - l) & 255) << (3 - x) * 8;
        else
            for (x = 0; x < 4; x++)
                c |= (ptwebqq.charCodeAt(j + x) & 255) << (3 - x) * 8;
        d ^= c;
    }
    a = [];
    a[0] = d >> 24 & 255;
